...
class sppb extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['no_sppb_baru'] = $this->M_sppb->generate_no_sppb();
    ...
    public function get_po_by_customer()
    {
        $id_customer = $this->input->post('id_customer');

        // Ambil semua PO yang masih outstanding berdasarkan customer
        $res_po = $this->M_po_customer->get_po_outstanding();

        $data_filtered = [];
        foreach ($res_po as $po) {
            if ($po['id_customer'] == $id_customer) {
                // hitung total SPPB keluar
                $d['id_mkt_po_customer'] = $po['id_mkt_po_customer'];
                $jml_sppb = $this->M_sppb->jml_sppb($d)->row_array();

                $sisa = $po['jumlah_po_customer'] - $jml_sppb['tot_sppb'];
                $po['tot_sppb'] = $jml_sppb['tot_sppb'];
                $po['sisa'] = $sisa;
