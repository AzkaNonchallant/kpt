...
class admin_sample_keluar extends MY_Controller {
    ...
    public function index()
    {
        ...
        $data['result'] = $this->M_admin_sample_keluar->get($tgl, $tgl2, $nama_barang, $nama_supplier);
        ...
        $this->template->load('template', 'content/gudang/gudang_admin/admin_sample_keluar_data', $data);
    }
    ...
    public function proses()
    {
        ...
        $data['kode_sample_out'] = $this->input->post('kode_sample_out', TRUE);
        ...
        $respon = $this->M_admin_sample_keluar->proses($data);

        if($respon){
            header('location:'.base_url('gudang/gudang_admin/admin_sample_keluar').'?alert=success&msg=Selamat anda berhasil menambah PO Sample');
        }else{
            header('location:'.base_url('gudang/gudang_admin/admin_sample_keluar').'?alert=danger&msg=Maaf anda gagal menambah PO Sample');
        }
    }
    ...
    public function update()
    {
        ...
        if($respon){
            header('location:'.base_url('gudang/gudang_admin/admin_sample_keluar').'?alert=success&msg=Selamat anda berhasil meng-update PO Sample');
        }else{
            header('location:'.base_url('gudang/gudang_admin/admin_sample_keluar').'?alert=danger&msg=Maaf anda gagal meng-update PO Sample');
        }
    }
    ...
    public function delete($id_mkt_po_sample)
    {
        ...
        $respon = $this->M_admin_sample_keluar->delete($data);

        if($respon){
            header('location:'.base_url('gudang/gudang_admin/admin_sample_keluar').'?alert=success&msg=Selamat anda berhasil menghapus PO Sample');
        }else{
            header('location:'.base_url('gudang/gudang_admin/admin_sample_keluar').'?alert=danger&msg=Maaf anda gagal menghapus PO Sample');
        }
    }
...
// }
    ...
    public function cek_kode_tf_in(){
        ...
        $row = $this->M_admin_sample_keluar->cek_kode_tf_in($kode_tf_in);
        if ($row['count_ktf'] == 0) {
            echo "false";
        } else {
            echo "true";
