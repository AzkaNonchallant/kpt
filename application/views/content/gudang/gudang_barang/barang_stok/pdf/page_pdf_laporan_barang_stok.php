  ...
  <table class="main">
    <thead>
      <tr>
        ...
        <th style="width:12%;">Kode Tf In</th>
        <th style="width:26%;">Nama Barang</th>
        <th style="width:26%;">No Batch</th>
    ...
    <tbody>
        ...
        $tgl_msk_gdg =  explode('-', $k['tgl_masuk'])[2]."/".explode('-', $k['tgl_masuk'])[1]."/".explode('-', $k['tgl_masuk'])[0];
        $tgl_exp =  explode('-', $k['tgl_exp'])[2]."/".explode('-', $k['tgl_exp'])[1]."/".explode('-', $k['tgl_exp'])[0];
        ?>
        <tr>
          <td class="text-center"><?= $no++ ?></td>
          <td class="text-left"><?= $k['no_po_pembelian'] ?></td>
          <td class="text-left"><?= $k['kode_tf_in'] ?></td>
          <td class="text-left"><?= strtoupper($k['nama_barang']) ?></td>
          <td class="text-left"><?= strtoupper($k['no_batch']) ?></td>
          <td class="text-left"><?= $tgl_msk_gdg ?></td>
          <td class="text-center"><?= $tgl_exp ?></td>
          <td class="text-center"><?= $k['jenis_transaksi_gudang'] ?></td>
          <td class="text-right"><?= number_format($k['total_masuk'], 0, ",", ".") ?> <?= $k['satuan'] ?></td>
          <td class="text-right"><?= number_format($k['total_keluar'], 0, ",", ".") ?> <?= $k['satuan'] ?></td>
          <td class="text-right"><?= number_format($k['stok_akhir'], 0, ",", ".") ?><?=$k['satuan']?></td>
        </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
